...
# Định nghĩa hàm để đổi đuôi file
def change_file_extension(old_file_path, new_extension):
    # Tách đường dẫn và tên file từ đường dẫn cũ
    folder_path, file_name = os.path.split(old_file_path)
    
    # Tách phần mở rộng cũ
    old_extension = os.path.splitext(file_name)[1]
    
    # Tạo đường dẫn mới với phần mở rộng mới
    new_file_path = os.path.join(folder_path, file_name.replace(old_extension, new_extension))
